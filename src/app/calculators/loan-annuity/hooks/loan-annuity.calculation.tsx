...
/**
 * Hook for handling loan annuity calculations
 * @returns Object containing calculation function and related state
 */
export function useLoanAnnuityCalculation() {
  const state = useLoanAnnuityState();
  const dispatch = useLoanAnnuityDispatch();

  /**
   * Perform the loan annuity calculation based on current form state
   */
  const calculateLoan = async () => {
    ...
    dispatch({ type: LoanAnnuityActionType.SetCalculating, payload: true });
    ...
    try {
      ...
      if (calculationMode === CalculationMode.ByTerm) {
        ...
        result = calculateAnnuityLoan(principalNum, interestRateNum, {
          months: monthsNum,
        });
      } else {
      ...
      dispatch({ type: LoanAnnuityActionType.SetResult, payload: result });
    } catch (error) {
      console.error("Calculation error:", error);
      dispatch({ type: LoanAnnuityActionType.SetResult, payload: null });
    } finally {
      dispatch({ type: LoanAnnuityActionType.SetCalculating, payload: false });
    }
  };
