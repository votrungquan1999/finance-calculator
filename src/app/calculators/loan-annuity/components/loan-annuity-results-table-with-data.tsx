...
import { useLoanAnnuitySummary } from "../hooks/loan-annuity.summary";
import { useLoanAnnuityState } from "../loan-annuity.state";
import { SummaryContainer, SummaryItem } from "../loan-annuity.ui";
import { generateShareableUrlState } from "../loan-annuity.url";

/**
 * Results table component that uses hooks to access calculator state
 * Renders the loan annuity payment schedule with calculated results and summary
 */
export function LoanAnnuityResultsTableWithData() {
  const state = useLoanAnnuityState();
  const { getSummary } = useLoanAnnuitySummary();
  ...
  const handleDownloadCSV = async () => {
    try {
      ...
      if (link.download !== undefined) {
        ...
        link.setAttribute("download", "loan-annuity-payment-schedule.csv");
