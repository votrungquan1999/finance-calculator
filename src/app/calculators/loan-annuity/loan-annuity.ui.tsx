...
import type { CalculationMode } from "./loan-annuity.type";
...
export function TabsWrapper({ children }: { children: React.ReactNode }) {
  ...
  const { handleModeChange } = useLoanAnnuityInput();
...
/**
 * Main container for the loan annuity calculator
 */
export function LoanAnnuityContainer({
  children,
}: {
...
/**
 * Hero section container for loan annuity calculator
 */
export function LoanAnnuityHero({ children }: { children: React.ReactNode }) {
  return <div className="space-y-4">{children}</div>;
}

/**
 * Form section container for loan annuity calculator
 */
export function LoanAnnuityFormSection({
  children,
}: {
  children: React.ReactNode;
}) {
...
/**
 * Form wrapper component for loan annuity calculator
 * Handles form submission and Enter key functionality
 */
export function LoanAnnuityFormWrapper({
  children,
}: {
  children: React.ReactNode;
}) {
  const { calculateLoan } = useLoanAnnuityCalculation();
...
/**
 * Results section container for loan annuity calculator
 */
export function LoanAnnuityResultsSection({
  children,
}: {
  children: React.ReactNode;
}) {
    ...
    <div className="bg-card text-card-foreground p-4 rounded-lg border">
      ...
      {isNumeric ? (
        <SaveValueDialog
          ...
          calculatorSource="Loan Annuity Calculator"
