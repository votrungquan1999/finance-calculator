...
import { useLoanDecliningSummary } from "../hooks/loan-declining.summary";
import { useLoanDecliningState } from "../loan-declining.state";
import { SummaryContainer, SummaryItem } from "../loan-declining.ui";
import { generateShareableUrlState } from "../loan-declining.url";

/**
 * Results table component that uses hooks to access calculator state
 * Renders the loan declining payment schedule with calculated results and summary
 */
export function LoanDecliningResultsTableWithData() {
  const state = useLoanDecliningState();
  const { getSummary } = useLoanDecliningSummary();
  ...
  const handleDownloadCSV = async () => {
    try {
      ...
      if (link.download !== undefined) {
        ...
        link.setAttribute("download", "loan-declining-payment-schedule.csv");
